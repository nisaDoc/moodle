...
if ($ADMIN->fulltree) {
    ...
    $settings->add(new admin_setting_configtext('forum_announcement_maxattachments', get_string('maxattachments', 'forum'),
                       get_string('configmaxattachments', 'forum'), 1, PARAM_INT));

    // Default Subscription mode setting for announcement forums.
    $options = forum_get_subscriptionmode_options();
    $settings->add(new admin_setting_configselect('forum_announcement_subscription', get_string('subscriptionmode', 'forum'),
